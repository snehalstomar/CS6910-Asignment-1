...
class feed_fwd_nn:
	...
	def train_nadam(self, data, epochs, batchsize, eta,beta1,beta2):
		...
		for epoch in range(epochs):
			...
			for batch in range(int(len(data)/batchsize)):
				...
				mw_hat = [ (i+(1-beta1)*j) / (1-math.pow(beta1,epoch+1))  for i,j in zip(self.mw,self.grad_Weights)]	
				mb_hat = [(i+(1-beta2)*j)/(1-math.pow(beta1,epoch+1)) for i,j in zip(self.mb,self.grad_Biases)]
	...
	def train(self):
		if self.optimizer == "sgd":
			...
			self.train_nadam(self.data, self.epochs, self.batchsize, self.eta, self.beta1,self.beta2)
