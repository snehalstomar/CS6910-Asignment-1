...
class feed_fwd_nn:
	...
	def train_adam(self, data, epochs, batchsize, eta,beta1,beta2):
		...
		for epoch in range(epochs):
			...
			for batch in range(int(len(data)/batchsize)):
				...
				VW = [i * 0 for i in self.vw]
				VB = [i * 0 for i in self.vb]
				MW = [i * 0 for i in self.mw]
				MB = [i * 0 for i in self.mb]
				...
				self.mw = [beta1*i + (1-beta1)* j for i,j in zip(MW,dW)]
				self.mb = [beta1*i + (1-beta1)* j for i,j in zip(MB,dB)]
				self.vw = [beta2*i + (1-beta2)*j**2 for i,j in zip(VW,dW)]
				self.vb = [beta2*i + (1-beta2)*j**2 for i,j in zip(VB,dB)]

				mw_hat = [j/(1-math.pow(beta1,i+epoch)) for j in self.mw]
				mb_hat = [j/(1-math.pow(beta1,i+epoch)) for j in self.mb]

				vw_hat = [j/(1-math.pow(beta2,i+epoch)) for j in self.vw]
				vb_hat = [j/(1-math.pow(beta2,i+epoch)) for j in self.vb]



				update_W = [i - (eta/np.sqrt(vw_hat+10**(-8))*j for i,j in zip(self.Weights,mw_hat))]
				#print(" length of update_W==>", len(update_W))
				#print("length of updateW====>", len(update_W))
				update_B = [i - (eta/np.sqrt(vb_hat+10**(-8))*j for i,j in zip(self.Biases,mb_hat))]
				...
				#return
				self.Weights = update_W    #[a - b for a, b in zip(self.Weights, update_W)]	
				...
				self.Biases = update_B #[a - b for a, b in zip(self.Biases, update_B)]
	...
	def train(self):
		if self.optimizer == "sgd":
			...
			train_adam(self.data, self.epochs, self.batchsize, self.eta, self.beta1,self.beta2)
