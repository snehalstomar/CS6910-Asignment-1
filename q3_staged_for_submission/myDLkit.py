...
Principal Python Header Containing defininiton of the Neural Network nad its allied functions
'''
import numpy as np

#The primary neural network class
class feed_fwd_nn:
	...
	#sub-class for definition of all hidden layers except the last one
	class hidden_layer:
	...
	#input layer sub-class(It defines an input layer  as taught in class)
	class input_layer:
	...
	#sub-class for defininition of hidden layer just before output.   		
	class layer_before_output:
		...
		def activation_sigmoid(self):
			for i in range(self.size_in):
				self.h[0, i] = 1 / (1 + np.exp(self.a[0, i]))
	#sub class for definition of output layer 	
	class output_layer:
	...
	#Definition of forward prop function 	
	...
	def backProp(self, input_vector, true_label):
		...
		self.error = -np.log(y_hat[0, true_label])
		y_actual = np.zeros([1, self.n_outputs])
		y_actual[0, true_label] = 1
		if np.array_equal(y_hat,y_actual):
