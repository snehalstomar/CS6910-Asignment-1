...
class feed_fwd_nn:
	...
	def __init__(self, number_neurons_in_layers, activation, training_parameters, number_layers: int = 1, initialization: str= "random"):
		...
		if self.init == "random":
			for i in range(self.num_layers):
				...
				#self.vw.append(np.zeros((self.neurons[i+1], self.neurons[i])))
				#self.vb.append(np.zeros(self.neurons[i+1], self.neurons[i]))
				#self.mw.append(np.zeros(self.neurons[i+1], self.neurons[i]))
				#self.mb.append(np.zeros(self.neurons[i+1], self.neurons[i]))

				self.vw.append(np.random.randn(self.neurons[i+1], self.neurons[i]))
				self.mw.append(np.random.randn(self.neurons[i+1], self.neurons[i]))
				self.vb.append(np.random.randn(self.neurons[i+1], 1))
				self.mb.append(np.random.randn(self.neurons[i+1], 1))
			...
			for i in range(self.num_layers):
				...
				#self.mw.append(np.zeros(self.neurons[i+1], self.neurons[i]))
				self.mw.append(np.random.randn(self.neurons[i+1], self.neurons[i])*np.sqrt(1/(self.neurons[i+1] + self.neurons[i+1])))

				#self.mb.append(np.zeros(self.neurons[i+1], self.neurons[i]))
				self.mb.append(np.random.randn(self.neurons[i+1], 1) * np.sqrt(1/(self.neurons[i+1] + self.neurons[i+1])))
	...
	def find_Loss(self, Y_pred, loss, true_label):
		...
		if loss == "cross-entropy":
			self.lossval = -np.log(Y_pred[true_label ,0])
			return self.lossval
		elif loss == "squared-error":
			self.lossval = np.linalg.norm(self.Y_true_one_hot - Y_pred) ** 2
			return self.lossval
		else:
	...
	def back_prop(self, input_feature_vector, loss_type, true_label):
		...
		self.given_label.append(true_label)
		self.pred_label.append(np.argmax(self.y_hat))
	...
	def train_mbgd(self, data, epochs, batchsize, eta,gamma):
		print("training using mbgd")
	...
	def train_nag(self, data, epochs, batchsize, eta,gamma):
		...
		for epoch in range(epochs):
			...
			for batch in range(int(len(data)/batchsize)):
				...
				#print( dW)
				dB = [i * 0 for i in self.Biases]
				VW = [i * gamma for i in self.vw]
				VB = [i * gamma for i in self.vb]
				#print("initial length of dB", len(dB))
				#print("initial length of dW", len(dW))
				#calculation of gradients
				for i in range(batchsize):
					...
					#print("length of grad_Weights in loop==>", len(self.grad_Weights))
					for j in range(len(self.grad_Weights)):
						dW[j] += self.grad_Weights[j] - VW[j]
					#dW += [i for i in self.grad_Weights]
					#print("lenth of dW in loop ==>", len(dW	))
					#print("length of grad_biases in loop==>", len(self.grad_Biases))
					for k in range(len(self.grad_Weights)):
						dB[k] += self.grad_Biases[k] - VB[k]
					#dB += [i for i in self.grad_Biases]
					#print("lenth of dB in loop ==>", len(dB))
					#print("dW==>",dW)
	...
	def train_nadam(self, data, epochs, batchsize, eta,beta1,beta2):
		print("training using nadam")
		for epoch in range(epochs):
			self.accuracy = 0
			print("==============in epoch: ", epoch+1, '=====================')
			#print("length of biases at start of training==>", len(self.Biases))
			...
			for batch in range(int(len(data)/batchsize)):
				...
				#print( dW)
				dB = [i  for i in self.grad_Biases]
				VW = [i  for i in self.vw]
				VB = [i for i in self.vb]
				#print("initial length of dB", len(dB))
				#print("initial length of dW", len(dW))
				#calculation of gradients
				for i in range(batchsize):
				...
				self.mw = [beta1*(i-1) + (1-beta1)*j for i,j in zip(self.mw,dW)]
				self.mb = [beta1*(i-1) + (1-beta1)*j for i,j in zip(self.mb,dB)]
				#mw = [mw[i+1] for i in range(len(mw))]

				self.vw = [beta2*(i-1) + (1-beta2)*j for i,j in zip(self.vw,dW)]
				self.vb = [beta2*(i-1) + (1-beta2)*j**2 for i,j in zip(self.vb,dB)]

				mw_hat = [i/(1-math.pow(beta1,epoch+1)) for i in self.mw]	
				mb_hat = [i/(1-math.pow(beta1,epoch+1)) for i in self.mb]

				vw_hat = [i/(1-math.pow(beta2,epoch+1)) for i in self.vw]
				vb_hat = [i/(1-math.pow(beta2,epoch+1)) for i in self.vb]
				...
				#print(" length of update_B==>", len(update_B))
				#return
				update_w = [np.sqrt(i + eta) * (j-1) for i,j in zip(vw_hat,mw_hat)]
				update_b = [np.sqrt(i + eta) * (j-1) for i,j in zip(vb_hat,mb_hat)]
	...
	def train_adam(self, data, epochs, batchsize, eta,beta1,beta2):
		...
		for epoch in range(epochs):
			...
			for batch in range(int(len(data)/batchsize)):
				...
				#update step
				self.mw = [beta1*i + (1-beta1)*j for i,j in zip(self.mw,dW)]
				self.mb = [beta1*i + (1-beta1)*j for i,j in zip(self.mb,dB)]

				self.vw = [beta2*i + (1-beta2)*j for i,j in zip(self.vw,dW)]
				self.vb = [beta2*i + (1-beta2)*j**2 for i,j in zip(self.vb,dB)]
				...
				#print(" length of update_B==>", len(update_B))
				#return
				update_w = [np.sqrt(i + eta) * j for i,j in zip(vw_hat,mw_hat)]
				update_b = [np.sqrt(i + eta) * j for i,j in zip(vb_hat,mb_hat)]
	...
	def train(self):
		if self.optimizer == "sgd":
			...
			self.train_adam(self.data, self.epochs, self.batchsize, self.eta, self.beta1,self.beta2)
